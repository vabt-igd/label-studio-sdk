...
class Converter(object):
    ...
    def convert(self, input_data, output_data, format, is_dir=True, **kwargs):
        ...
        if format == Format.JSON:
            ...
            from label_studio_sdk.converter.exports.doclang import (
                convert_to_doclang,
                resolve_image_data_keys,
            )

            image_key = kwargs.get("image_key")
            image_list_key = kwargs.get("image_list_key")
            if image_key is None:
                single_key, list_key = resolve_image_data_keys(
                    self._schema, self._config_string
                )
    ...
    def _get_supported_formats(self):
        ...
        supports_doclang = any(
            info.get("type") in {"TextArea", "ReactCode", "CustomInterface"}
